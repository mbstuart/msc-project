...
class ArticlePreprocessJob(BaseJob):
    ...
    def preprocess_articles_update(self, load_id: str, from_load_id: str):
        ...
        preprocessed_articles = self.preprocess_raw_articles_update(articles, from_load_id, load_id);
    ...
    def get_articles_for_update(self, load_id: str, from_load_id: str) -> List[Article]:
        session: Session = self.get_session();
        from_article: Article = session.query(Article).filter_by(article_load_id = from_load_id).order_by(desc(Article.publish_date)).first()
        query = session.query(Article).filter_by(article_load_id = load_id).filter(Article.publish_date > from_article.publish_date)
        articles = query.all();

        return articles

    def preprocess_raw_articles(self, raw_articles: List[Article], load_id: str):
        preprocessed_articles = self.preprocessor.preprocess_articles(raw_articles, load_id);
        return preprocessed_articles;

    def preprocess_raw_articles_update(self, raw_articles: List[Article], old_load_id: str, new_load_id: str):
        preprocessed_articles = self.preprocessor.preprocess_articles_update(raw_articles, old_load_id, new_load_id);
        return preprocessed_articles;

    def clone_preprocessed_articles_to_new_load_id(self, old_load_id: str, new_load_id: str):
        session: Session = self.get_session();

        process_articles: List[ProcessedArticle] = session.query(ProcessedArticle).filter_by(article_load_id = old_load_id).all()

        cloned_articles: List[ProcessedArticle] = [pa.clone(new_load_id) for pa in process_articles];

        session.bulk_save_objects(cloned_articles);
