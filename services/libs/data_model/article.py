...
class Article(Base):
    ...
    source_tags = Column('SourceTags', ARRAY(String))
    publish_date = Column('PublishDate', DateTime)
    article_load_id = Column('ArticleLoadId', UUID(
        as_uuid=True), primary_key=True, default=uuid.uuid4, unique=True, nullable=False)

    def __init__(self, id, body, title, source_tags, publish_date, article_load_id):
        self.id = id
        self.body = body
        self.title = title
    ...
    def clone(self, load_id=None):
        ...
        return Article(self.id, self.body, self.title, self.source_tags, self.publish_date, load_id)
